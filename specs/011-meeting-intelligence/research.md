...
**Decision.** The 2026-09-21 follow-up reuses the meeting-language selection introduced by Feature 009. A fixed supported language recorded in the final pass identity wins (it is what was transcribed); otherwise an explicit Slovak or English meeting choice. Otherwise `LanguagePolicy.detect` samples up to 32 KiB of final segment text spread across the meeting, runs an English/Slovak-constrained `NLLanguageRecognizer` on each sampled segment, and yields `sk`, `en` or `mixed` (`mixed` when no supported language reaches 80% of sampled characters, except that English with at least twice the Slovak characters yields `en`, since `mixed` is Slovak prose). Samples end at valid UTF-8 boundaries. Automatic keeps text detection. LocalFlow supports English and Slovak only (2026-09-24: Czech removed). The request carries `language_policy: {"output": "sk"|"en"|"mixed", "preserve_terms": true}`. `mixed` instructs Slovak prose with English technical terms kept.
