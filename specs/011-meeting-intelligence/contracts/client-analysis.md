...
4. **Language policy.** `LanguagePolicy.detect` on a bounded sample (R10).
...
| protected literals | `ProtectedLiteralDetector` per item and topic against referenced sources; for the summary against all evidence | item dropped, count `dropped_literal`; summary failure → run fails `protected_literal` |
...
| chunk text | 24,576 B (or server `limits.input_bytes` if smaller) |
| chunks per run | 64 |
