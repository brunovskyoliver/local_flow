...
Same as rewrite: HTTP/1.1, UTF-8 JSON bodies, `Authorization: Bearer <secret>` when the endpoint has a credential, keep-alive, no compression, limits on raw bytes. The server rejects request bodies over 262,144 bytes with `413`; the client stops reading a response at a line over 98,304 bytes or a stream over 524,288 bytes (progress lines every 250 ms for the 300 s request timeout plus the result line) and records `oversized_response`. Every request carries `priority: "background"`; the server uses it for admission ordering and preemption (R4). A rewrite request never waits for an analysis request.
