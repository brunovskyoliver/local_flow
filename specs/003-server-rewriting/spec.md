...
- **FR-004**: The rewrite request MAY contain only: the faithful transcript text, selected mode, protocol/schema version, a request identifier, locally known language hints, and required feature flags. It MUST NOT contain microphone or system audio, other history entries, meeting data, clipboard contents, active-application contents, surrounding target text or filesystem data. Application-aware context is out of scope. *Superseded in part by Feature 012 ([ADR 0023](../../docs/adr/0023-rewrite-protocol-v2-reference-context.md)): when the user turns on both context toggles and the server supports protocol v2, the request may also carry one bounded, redacted snapshot of the focused field and window title, under the rules of `specs/012-app-context-awareness/contracts/rewrite-protocol-v2.md`. Every other exclusion above still holds.*
