...
`result` identity fields (`server`, `backend`, `prompt_version`, `shield`) are required in v1 so every stored attempt and every corpus result is reproducible. `timing` is required; a span the server cannot measure is omitted, never zero. `shield.placeholders` and `shield.restored` are counts only; the server sends `result` only when they are equal.
