...
final class ServerEndpointTests: XCTestCase {
    ...
    func testEnvironmentOverrideNormalizesBeforeCredentialLookup() throws {
        ...
        var accounts = [String]()
        let store = ClientPreferencesStore(root: root, environment: ["SOTTO_SERVER_URL": " \nhttps://EXAMPLE.com:443/sotto/// \n"],
                                          readCredential: { accounts.append($0); return "saved-key" })
        XCTAssertEqual(store.endpoint, "https://EXAMPLE.com:443/sotto")
        XCTAssertEqual(accounts, [root.standardizedFileURL.path + "|https://EXAMPLE.com:443/sotto"])
    ...
    func testInvalidOverrideNeverLoadsCredentialsAndSaveUsesTheSamePolicy() {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        var reads = 0
        let store = ClientPreferencesStore(root: root, environment: ["SOTTO_SERVER_URL": "http://example.com"],
