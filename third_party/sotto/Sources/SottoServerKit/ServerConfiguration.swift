...
public struct ServerConfiguration: Sendable {
    ...
    public static func parse(arguments: [String] = Array(CommandLine.arguments.dropFirst()),
        ...
        var development = environment["SOTTO_DEV"] == "1"
        ...
        let rawPort = value("port", "SOTTO_SERVER_PORT") ?? "8391"
        guard let port = Int(rawPort) else { throw ServerConfigurationError.invalid("Invalid port: \(rawPort)") }
        let token: String?
        if let tokenFile = value("token-file", "SOTTO_SERVER_TOKEN_FILE") {
        ...
        return try Self(host: value("host", "SOTTO_SERVER_HOST") ?? "127.0.0.1", port: port,
                        dataDirectory: path("data-dir", "SOTTO_SERVER_DATA_DIR"), token: token,
                        development: development,
                        inference: InferenceConfiguration(speechHelper: path("speech-helper", "SOTTO_ENGINE_PATH"),
                            speechModel: path("speech-model", "SOTTO_SPEECH_MODEL"), vadModel: path("vad-model", "SOTTO_VAD_PATH"),
                            proofHelper: path("proof-helper", "SOTTO_TEXT_ENGINE_PATH"), proofModel: path("proof-model", "SOTTO_TEXT_MODEL")))
    }

    public static let usage = """
    Sotto server — independent dictation service
