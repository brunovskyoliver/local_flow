...
func testBody(text string) string {
	b, _ := json.Marshal(Request{1, testID, "clean", text, []string{}, false})
	return string(b)
}
...
func TestRejections(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{strings.Replace(valid, "\"schema_version\":1", "\"schema_version\":2", 1), "Bearer secret", 400, CodeUnsupportedVersion},
